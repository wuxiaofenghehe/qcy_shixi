...
def json_to_yolo(json_path, img_path, save_path, class_names=None, format_type="auto"):
    ...
    for idx, shape in enumerate(shapes):
        ...
        print(
            f"  [对象{idx+1}] 类别: {label}, 形状类型: {shape_type}, 点数: {len(points)}"
        )
        ...
        # ===== 处理多边形（实例分割格式）=====
            ...
            yolo_line = f"{class_id} " + " ".join(
                [f"{coord:.6f}" for coord in normalized_coords]
            )
            print(
                f"转换为YOLO polygon格式: class={class_id}, {len(polygon_points)}个点"
            )
            yolo_lines.append(yolo_line)
...
if __name__ == "__main__":
    # 对于分割和检测需要更改类别名称
    batch_convert(
        img_dir=r"D:\wellimg\wellimg20260401\202260401sliced-data\total-det-data-json",  # 图片文件夹路径
        json_dir=r"D:\wellimg\wellimg20260401\202260401sliced-data\total-det-data-json",  # JSON标签文件夹路径
        out_dir=r"D:\wellimg\wellimg20260401\202260401sliced-data\total-det-data-yolo",  # 输出YOLO格式标签的文件夹路径
        # class_names=["gdf", "gzf"],  # 类别名称列表，可以包含中文
        class_names=["bl", "yl", "yz"],  # 类别名称列表
